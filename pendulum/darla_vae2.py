...
def sampling(args):
    ...
    epsilon = K.random_normal(shape=(batch, 32))
...
class DataSequence(Sequence):
    def __init__(self):
        self.filenames = glob.glob("vae_car_data_small/*.npz")
        self.image_size = 32
        self.i = 0
        self.batch_size = 100
        self.npz_idx = 0
    ...
    def __getitem__(self, idx):
        if (self.i * self.batch_size) > self.npz_len:
            self.i = 0
            self.npz_idx += 1
            if self.npz_idx >= len(self.filenames):
                self.npz_idx = 0
            self.data = np.load(self.filenames[self.npz_idx])
            self.images = self.data["images"]

        batch_x = self.images[self.i*self.batch_size:(self.i+1)*self.batch_size, :, :, :]
        self.i += 1
...
x_decoder = Lambda(lambda x: sampling(x))([decoder_input_mean, decoder_input_log_var])
x_decoder = Dense(256, activation='relu')(x_decoder)
...
x_decoder = Conv2DTranspose(filters=3, kernel_size=1, strides=1, activation='relu', padding='same')(x_decoder)
# x_decoder = Lambda(lambda x: x[:, :32, :32, :])(x_decoder)
...
# Reconstruction loss
sampled_reconstructions = outputs[0]
...
if __name__ == '__main__':
    ...
    darla_vae.load_weights("darla_vae_small3.h5")
    if True:
        darla_vae.summary()
        history = darla_vae.fit_generator(img_generator, epochs=epochs, workers=9)
        darla_vae.save_weights("darla_vae_small3.h5")
        darla_vae.save("full_darla_vae_small3.h5")
...
# Load Data
        data = np.load(glob.glob("vae_car_data_small/*.npz")[0])
        ...
        if False:
            for j in trange(0, 32):
                ...
                for i in range(51):
                    ...
                    if j < 10:
                        str_j = "0" + str_j
                    cv2.imwrite("sweep/viz" + str_j + "_" + str_i + ".png", cv2.resize(predicted_img, (512, 512)))
                predicted_means[:, j] = predicted_originals

        if True:
            for i in range(0, len(predictions[0]), 100):
                img = observations[i]
                cv2.imwrite("originals/original" + str(i) + ".png", cv2.resize(img, (512, 512)))
                predicted = np.clip(darla_vae.predict(np.expand_dims(img, axis=0))[0][0, :, :, :3], 0., 255.)
                cv2.imwrite("reconstructions/recon" + str(i) + ".png", cv2.resize(predicted, (512, 512)))
