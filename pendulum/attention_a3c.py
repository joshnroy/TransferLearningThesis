...
LOSS_ATTENTION = 0.01 #0.01
LOSS_PSEUDO_ATTENTION = 0.01
...
class Brain:
        ...
        def _build_model(self, test):
                ...
                vae.load_weights("darla_vae2.h5")
        ...
        def _build_graph(self, model):
                ...
                self.loss_attention = tf.reduce_sum(LOSS_ATTENTION * tf.abs(attention_weights))

                # PSEUDO ATTENTION
                # self.loss_attention += tf.reduce_mean(LOSS_PSEUDO_ATTENTION * tf.nn.moments(attention_weights, 0)[1])
        ...
        def predict_p(self, s):
                with self.default_graph.as_default():
                        ...
                        # sys.stderr.write(str(attention_weights) + "\n")
...
def test_a3c(brain, test=False):
    ...
    for e in trange(100):
        ...
        e_reward = 0
        while True:
            p, attention_weights = brain.predict_p(np.expand_dims(observation, axis=0))
            # sys.stderr.write(str(attention_weights) + "\n")
            p = p[0]
